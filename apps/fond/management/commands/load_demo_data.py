...
class Command(BaseCommand):
    ...
    def _load_funds(self):
        ...
        items = [
            ...
            {'title': 'Орден Трудового Красного Знамени, врученный коллективу', 'item_type': ItemType.OBJECT,
             'period': Period.P1945, 'kp_number': 'КП 41100', 'inventory_number': 'ГМИ СПб Инв.№-V-А-100',
    ...
    def _load_map(self):
        ...
        buildings = [
            {'slug': 'lifestak', 'name': 'Лайф-стак', 'built_years': '1931-1933',
             'page_title': '«Лайв-стак»',
             'page_subtitle': '(корпус предубойного содержания скота)',
             'page_status': 'ОКН',
             'page_description': 'Является важным звеном в технологической цепочке мясоперерабатывающего комплекса. В этом корпусе содержался перед убоем скот, откармливался, проходил ветеринарный осмотр. На 1 этаже здания находились ветеринарные службы и административные помещения. Остальные предназначались для содержания скота. Корпус оснащен специальным ступопандусом для подъема на этажи животных.',
    ...
    def _load_gallery(self):
        from apps.gallery.models import Album, GalleryPeriod, Media
        
        p1, _ = GalleryPeriod.objects.get_or_create(title='До 1930 года', defaults={'order': 1, 'date_range': '1880–1930'})
        p2, _ = GalleryPeriod.objects.get_or_create(title='Строительство комбината', defaults={'order': 2, 'date_range': '1931–1933'})
        p3, _ = GalleryPeriod.objects.get_or_create(title='Предвоенные годы', defaults={'order': 3, 'date_range': '1934–1940'})
        p4, _ = GalleryPeriod.objects.get_or_create(title='Великая Отечественная война', defaults={'order': 4, 'date_range': '1941–1945'})
        p5, _ = GalleryPeriod.objects.get_or_create(title='Золотой век комбината', defaults={'order': 5, 'date_range': '1946–1990'})
        p6, _ = GalleryPeriod.objects.get_or_create(title='Новейшая история', defaults={'order': 6, 'date_range': '1991–н/в'})

        albums = [
            {'title': 'Первые строители комбината', 'gallery_period': p1, 'order': 1},
            {'title': 'Строительство главного корпуса', 'gallery_period': p2, 'order': 1},
            {'title': 'Торжественное открытие, 1933', 'gallery_period': p2, 'order': 2},
            {'title': 'Рабочие будни 1930-х', 'gallery_period': p3, 'order': 1},
            {'title': 'Стахановцы комбината', 'gallery_period': p3, 'order': 2},
            {'title': 'Комбинат в годы блокады', 'gallery_period': p4, 'order': 1},
            {'title': 'Восстановление предприятия, 1944–1950', 'gallery_period': p5, 'order': 1},
            {'title': 'Юбилей 50 лет комбинату, 1980', 'gallery_period': p5, 'order': 2},
            {'title': 'Реорганизация предприятия', 'gallery_period': p6, 'order': 1},
            {'title': 'Музей трудовой и воинской славы', 'gallery_period': p6, 'order': 1},
        ]
        for a in albums:
